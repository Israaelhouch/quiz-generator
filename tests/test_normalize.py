...
def test_mathematics_subject_mislabeled_english_gets_overridden_to_fr() -> None:
    ...
    # The canonical mislabel (English flag, French math content) must end
    # up classified as French. We don't pin WHICH rule fixed it — either
    # script detection or the subject-based domain rule is acceptable;
    # what matters is the production outcome.
    assert normalized["language"] == "fr"
    # And the audit trail must record that some non-identity correction
    # happened (it shouldn't silently trust the bad "english" raw value).
    assert audit["language_source"] != "raw"
