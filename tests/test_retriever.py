...
def test_empty_store_signal_repeats_every_call() -> None:
...
class _FakeReranker:
    ...
    Signature mirrors the real Reranker.rerank() including the optional
    `scores_out` out-parameter (used by retriever diagnostics). When
    provided, we fill it with the scores in returned order so the
    diagnostics-coupling tests can run against this fake too.
    """
    ...
    def rerank(
        ...
        *,
        ...
        if scores_out is not None:
            scores_out.clear()
            scores_out.extend(float(s) for _, s in scored)
...
def test_retrieve_serialises_concurrent_callers() -> None:
    ...
    and neither SentenceTransformer nor CrossEncoder promises thread-safety.
    retrieve() must therefore admit one caller at a time."""
    import threading
    ...
    class _ObservingModel:
        """Records the maximum number of threads inside the ML section."""
    ...
    assert state["peak"] == 1, (
        f"{state['peak']} threads were inside the ML section at once — "
        "the lock in Retriever.retrieve is not holding"
    )


def test_retrieve_wrapper_forwards_args_and_kwargs() -> None:
    """The lock wrapper uses *args/**kwargs — make sure nothing is dropped."""
    payload = {"id-1": _payload_row("id-1", quiz_title="T1")}
    r = _make_retriever(ids=["id-1"], distances=[0.1], payload=payload)

    positional = r.retrieve("q", language="en", top_k=1)
    assert len(positional) == 1
    assert positional[0].doc_id == "id-1"
