...
def test_pipeline_low_pool_warns_but_proceeds(tmp_path: Path) -> None:
    ...
    assert any("low retrieval pool" in m for m in messages), messages
...
def test_pipeline_multi_levels_warns_about_first_only(tmp_path: Path) -> None:
    ...
    assert any("forwarding only the first" in m for m in messages), messages
...
# ---------------------------------------------------------------------------
# Concurrency — cross-request data leak (regression)
# ---------------------------------------------------------------------------
# ONE QuizPipeline instance serves every HTTP request (built once at startup,
# called from FastAPI's thread pool). Anything a caller needs after generate()
# returns must come back ON THE RETURN VALUE. The old code stashed the
# retrieval on self.last_retrieval and the endpoint read it afterwards, so a
# concurrent call could overwrite it in between — teacher A's response, and
# A's runs.jsonl entry, would carry teacher B's source questions.
#
...
def test_generate_detailed_is_isolated_across_threads(tmp_path: Path) -> None:
    ...
    class _SlowPerTopicRetriever:
        """Returns a doc named after the query, slowly — so the window
        between 'retrieve finished' and 'caller reads it' stays open long
        enough for other threads to interleave."""
        ...
        def retrieve(self, **kwargs):
            _time.sleep(0.02)
            self.calls.append(kwargs)
            return [_retrieved(f"{kwargs['query']}-doc")]
    ...
    pipeline = QuizPipeline(
        config_path=cfg,
        _retriever=_SlowPerTopicRetriever(),
        _llm_client=_SlowLLM(_VALID_LLM_RESPONSE),
    )
    ...
    threads = [threading.Thread(target=_run, args=(t,)) for t in topics]
    for th in threads:
        th.start()
    for th in threads:
        th.join(timeout=30)
    ...
    for topic in topics:
        assert seen[topic] == [f"{topic}-doc"], (
            f"cross-request leak: {topic!r} received {seen[topic]}"
        )
    # Timings are per-call too, not shared.
    assert all(not isinstance(v, type(None)) for v in seen.values())


def test_generate_wrapper_still_returns_quiz_and_mirrors_state(tmp_path: Path) -> None:
    """generate() keeps its old signature for the CLI: returns the quiz and
    mirrors retrieval/timings onto the instance."""
    cfg = _write_minimal_config(tmp_path)
    pipeline = QuizPipeline(
        config_path=cfg,
        _retriever=_FakeRetriever([_retrieved("ex-A")]),
        _llm_client=MockClient(canned_response=_VALID_LLM_RESPONSE),
    )
    with warnings.catch_warnings():
        warnings.simplefilter("ignore")
        quiz = pipeline.generate(topic="x", language="en", count=1, few_shot_count=1)

    assert isinstance(quiz, GeneratedQuiz)
    assert [c.doc_id for c in pipeline.last_retrieval] == ["ex-A"]
    assert pipeline.last_timings["n_examples_used"] == 1


def test_generate_detailed_returns_all_three_parts(tmp_path: Path) -> None:
    cfg = _write_minimal_config(tmp_path)
    pipeline = QuizPipeline(
        config_path=cfg,
        _retriever=_FakeRetriever([_retrieved("ex-A"), _retrieved("ex-B")]),
        _llm_client=MockClient(canned_response=_VALID_LLM_RESPONSE),
    )
    with warnings.catch_warnings():
        warnings.simplefilter("ignore")
