...
# requirements.txt records INTENT (what we depend on).
# requirements.lock.txt records EXACT versions, generated with:
#     docker compose run --rm api pip freeze > requirements.lock.txt
# The image builds from the lock so a rebuild months from now cannot silently
# change the embedding model, torch, or an LLM SDK under us.
