"""Generation orchestrator — retriever + prompt + LLM + validation, with retry.
...
class Generator:
    ...
    def _parse_and_validate(
        ...
        # LaTeX-validity check. The LLM occasionally emits broken LaTeX
        # (missing brace, unclosed \(...\), etc.) that would fail to
        # render on the frontend. Catch it here so the retry can fix it
        # instead of shipping garbage to the user.
        for idx, q_obj in enumerate(validated):
            ...
            for j, ans in enumerate(q_obj.correct_answers or []):
                checkable.append((f"correct_answers[{j}]", ans or ""))
            for field_name, value in checkable:
                ok, reason = check_latex_validity(value)
                if not ok:
                    return (
                        f"Question {idx} {field_name}: invalid LaTeX — "
                        f"{reason}. Fix the LaTeX so it renders. Use "
                        f"\\(...\\) for inline math, balance every {{ and }}."
                    )
