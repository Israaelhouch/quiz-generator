...
The ingest module produces FlatQuestion. the normalize module produces CleanedQuestion.
"""
...
class FlatQuestion(BaseModel):
    """Ingest output — one row per kept question. NOT cleaned yet.

    Text fields are suffixed `_raw` to mark that HTML stripping and
    whitespace normalization happen in the normalize module.
    """
...
class NormalizedQuestion(BaseModel):
    """Normalize output — one cleaned, language-normalized, dedup'd row.
...
class IndexedQuestion(BaseModel):
    """Index-text output — the embedding-ready row.
    ...
    (like 'passage: ' for e5) are added at embed time in the indexing module, not here,
