...
from src.data.curriculum_rules import check_compliance as check_curriculum_compliance
...
def normalize_row(flat: dict, aliases: dict[str, str]) -> tuple[dict | None, str, dict]:
    ...
    # drop it. the ingest module's empty_choices check catches zero-length lists; this
    # catches lists with entries that are all blank strings after HTML strip.
    question_type_val = str(flat.get("question_type") or "")
    # After the ingest module's TMC→MCQ merge, only MCQ needs the empty-choices check.
    ...
    school_phase = derive_school_phase(levels)

    # Curriculum compliance: e.g. Tunisian math is Arabic in primary/middle,
    # French in high school. Rows violating the curriculum (PRIMARY×fr math,
    # etc.) are usually source mistags and shouldn't be indexed. See
    # `src/data/curriculum_rules.py` for the rule table.
    compliant, curriculum_reason = check_curriculum_compliance(
        subjects=subjects,
        school_phase=school_phase,
        language=final_language,
    )
    if not compliant:
        return None, curriculum_reason, audit
...
def derive_school_phase(levels: list[str]) -> str | None:
    """Derive a coarse school-phase grouping from the first fine-grained level.

    Maps:
      PRIMARY_SCHOOL_*   → "PRIMARY"
      MIDDLE_SCHOOL_*    → "MIDDLE"
      HIGH_SCHOOL_*      → "HIGH"
      anything else      → None

    Returns None if levels is empty or first level doesn't match a known prefix.
    """
    if not levels:
        return None
    first = str(levels[0])
    if first.startswith("PRIMARY_SCHOOL"):
        return "PRIMARY"
    if first.startswith("MIDDLE_SCHOOL"):
        return "MIDDLE"
    if first.startswith("HIGH_SCHOOL"):
        return "HIGH"
    return None
