"""CLI shim around QuizPipeline.

Usage:
    python -m src.pipeline "past simple vs past continuous" \\
        --language en --subject ENGLISH --count 5
    python -m src.pipeline "الاستثناء" --language ar --subject ARABIC \\
        --count 5 --format json

Note: current scope supports en/ar/fr (literature + grammar). Mathematics
is deferred until LaTeX/SymPy infrastructure lands — see
docs/cells_plan.md for scope details.
"""
...
def _parse_args() -> argparse.Namespace:
    ...
    p.add_argument(
        "--levels",
        help="Comma-separated levels for retrieval filter (e.g. HIGH_SCHOOL_4TH_GRADE_ENGLISH)",
    )
    p.add_argument(
        "--few-shot",
        type=int,
...
def render_retrieval_human(retrieval: list, *, topic: str) -> str:
    ...
    lines.append(line)
    lines.append(
        f"RETRIEVED CHUNKS — what BGE-M3 fed to the LLM as few-shot examples (topic: {topic!r})"
    )
    ...
    for i, c in enumerate(retrieval, start=1):
        ...
        lines.append(
            f"    language   : {c.language}   type: {c.question_type}   subjects: {c.subjects}"
        )
...
def save_run_to_file(
    *,
    ...
    Contains the retrieved chunks (what BGE-M3 fed the LLM), the generated
    quiz (what the LLM produced), and per-stage timings. One file,
    overwritten each run, so you can always answer:
        - 'what did the LLM produce?'        → look at `quiz.questions`
        - 'what was the LLM looking at?'     → look at `retrieval`
        - 'where did the time go?'           → look at `timings`
    """
    payload = {
        "topic": topic,
...
def render_human(quiz: Any, *, topic: str) -> str:
    ...
    lines.append(
        f"language: {quiz.language}   subject: {quiz.subject or '-'}   level: {quiz.level or '-'}"
    )
...
def render_json(quiz: Any, *, topic: str, retrieval: list | None = None) -> str:
    ...
    evaluation framework eval will need both the generated quiz and the retrieval used to
    produce it, so we include retrieval whenever it's provided. When the CLI
    is in JSON mode the CLI always passes the retrieval from the
    GenerationResult it just received.
    """
    payload = {
        "topic": topic,
...
def main() -> None:
    ...
    print("Loading pipeline (this also loads the embedder and reranker)…", file=sys.stderr)
    ...
    if args.format == "json":
        # JSON always includes retrieval — evaluation framework eval needs it.
    ...
    # Per-stage timing footer — visible to the user, structured, easy to grep.
    if timings is not None:
        print(
            f"\n[timing] retrieve={timings['retrieve_seconds']:>6.2f}s  "
            f"generate={timings['generate_seconds']:>6.2f}s  "
            f"total={timings['total_seconds']:>6.2f}s  "
            f"(few_shot_used={timings['n_examples_used']})",
            file=sys.stderr,
        )
