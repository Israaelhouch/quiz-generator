...
Wraps the indexing module's vector store into a typed, filtered, deduplicated retrieval
API suitable for the LLM generator. Rewritten from scratch on top of the
...
NOT responsible for (kept elsewhere by design):
  - Filter relaxation fallback (deferred; may add in evaluation framework)
...
SCHOOL_PHASES = ("PRIMARY", "MIDDLE", "HIGH")
...
def _build_where(
    *,
    ...
    `school_phase` is the coarse education stage ("PRIMARY", "MIDDLE",
    "HIGH"). Each row carries it as a scalar metadata field (derived in
    normalize from `levels[0]`), so Chroma pre-filters natively. Cheaper
    and friendlier to the platform than enumerating every specific grade
    in `levels`.
    """
    ...
    if school_phase:
        clauses.append({"school_phase": school_phase})
    if levels:
...
class Retriever:
    ...
    def __init__(
        ...
        *,
        ...
        # Held across embed + Chroma query + rerank; deliberately NOT held
        # during the LLM call, which is network-bound and stays concurrent.
        # Set before the test-injection early-return below.
        ...
        self._taxonomy = Taxonomy.from_build_summary(
    ...
    def retrieve(self, *args: Any, **kwargs: Any) -> list[RetrievedQuestion]:
        """Thread-safe entry point — see `_retrieve_unlocked` for the args.

        Retrieval is CPU-bound and serialised. At ~0.5s (MPS) to ~5s (Docker
        CPU) per call this caps concurrent throughput, which is the correct
        trade against two threads corrupting shared model state. If retrieval
        becomes the bottleneck, run multiple workers (each gets its own models,
        ~1.2 GB apiece) rather than removing this lock.
        """
        with self._ml_lock:
            return self._retrieve_unlocked(*args, **kwargs)
    ...
    def _retrieve_unlocked(
        ...
        *,
        ...
        If `diagnostics` is provided, the dict is populated with intermediate
        state useful for offline analysis (eval, threshold tuning):

          * ``bi_encoder_doc_ids``  — full Chroma top-N result (pre-filter,
            pre-rerank), sorted ascending by cosine distance.
          * ``bi_encoder_distances`` — parallel list of distances.
          * ``pool_size`` — the n_results used for the Chroma query
            (depends on whether the reranker is enabled).

        The diagnostics out-parameter is intentionally optional so production
        callers don't pay any cost; only eval scripts populate it.
        """
        ...
        if school_phase is not None:
            ...
            if school_phase_normalized not in SCHOOL_PHASES:
                raise ValueError(
                    f"school_phase must be one of {SCHOOL_PHASES} (got {school_phase!r})"
                )
        else:
            school_phase_normalized = None
        ...
        where = _build_where(
            ...
            school_phase=school_phase_normalized,
        ...
        _t_chroma_start = _time_r.perf_counter()
        ...
        _t_chroma = _time_r.perf_counter() - _t_chroma_start

        ids = (results.get("ids") or [[]])[0]
        distances = (results.get("distances") or [[]])[0]

        # Diagnostics hook — snapshot raw Chroma output before any
        # post-filtering so offline analysis can simulate larger pools or
        # different distance thresholds without re-running retrieval.
        if diagnostics is not None:
            diagnostics["bi_encoder_doc_ids"] = list(ids)
            diagnostics["bi_encoder_distances"] = [float(d) for d in distances]
            diagnostics["pool_size"] = pool_size
        ...
        # analysis needs every (doc_id, score) pair, not just the top-k that
        # reaches the user. With reranker off, this is the post-filter pool in
        # distance-ascending order; with reranker on, it's reordered by score
        # descending.
        full_pool_doc_ids = [r.doc_id for r in output]

        # Slice to top_k after reranking (or after early break in no-reranker path).
        output = output[:top_k]

        # Diagnostics — multiple complementary views of the retrieval state.
        if diagnostics is not None:
            diagnostics["reranker_enabled"] = self._reranker is not None
            # Full post-rerank pool (parallel arrays, score-descending when
            # reranker is on, distance-ascending when off):
            diagnostics["reranker_full_doc_ids"] = full_pool_doc_ids
            diagnostics["reranker_full_scores"] = list(rerank_scores_pre_slice)
            # Reranker scores aligned with the FINAL returned top-k order
            # (subset of reranker_full_scores). Empty list when rerank is off.
            diagnostics["reranker_scores"] = rerank_scores_pre_slice[: len(output)]
